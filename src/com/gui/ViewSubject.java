...
//    private String phone;
    ...
    private void createTable(){
        String sql="select sub_name, sub_faculty from subject";

        JTable table;
        TableColumn column = null;
        JScrollPane scrollPane;
        String meta[]={"Subject name", "Subject faculty"};
        table= MyTable.createTable(sql, meta);
        table.setPreferredScrollableViewportSize(new Dimension(480, 200));
        table.setToolTipText("Select subject record and click edit or delete to edit or delete information");
        ...
        table.addMouseListener(new java.awt.event.MouseAdapter() {
            public void mouseClicked(java.awt.event.MouseEvent evt) {
                try{
                    ...
                    subject_name = table.getModel().getValueAt(row, 0).toString();
//                    phone = table.getModel().getValueAt(row, 1).toString();
                    subject_faculty = table.getModel().getValueAt(row, 1).toString();
    ...
    private void initComponents() {
        ...
        jLabel1.setText("EDIT/DELETE SUBJECT INFORMATION");
    ...
    private void deleteActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_deleteActionPerformed
        try{
            int confirm = JOptionPane.showConfirmDialog(this, "Delete subject Information? Notice that all information related to the subject will also be deleted.", "Confirmation", JOptionPane.YES_NO_OPTION);
            PreparedStatement st;

            if(confirm == JOptionPane.YES_OPTION){
                String sql = "delete from subject where sub_name = ? AND sub_faculty = ?";
                st = DBConnect.con.prepareStatement(sql);

                st.setString(1, subject_name);
                st.setString(2, subject_faculty);
//                st.setString(3, gender);

                st.executeUpdate();

                JOptionPane.showMessageDialog(this, "Subject information has been deleted","Success",JOptionPane.INFORMATION_MESSAGE);
    ...
    private void editActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_editActionPerformed
        try{
            PreparedStatement st;
            String sql = "select * from subject where sub_name = ? AND sub_faculty = ?";
            st = DBConnect.con.prepareStatement(sql);
...
//            st.setString(2, phone);
            ...
            EditSubject edit_subject = new EditSubject(sub_id);
            desktop1.add(edit_subject, new org.netbeans.lib.awtextra.AbsoluteConstraints(0, 0));
            edit_subject.setSelected(true);
        }catch (Exception e) {
            System.err.println(e.getMessage());
            e.printStackTrace();
        }
