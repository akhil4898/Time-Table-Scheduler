...
public class ViewLecturer extends javax.swing.JInternalFrame {
    ...
    private void createTable(){
        String sql="select instructor_name, phone, gender from instructor";
        ...
        table.addMouseListener(new java.awt.event.MouseAdapter() {
            public void mouseClicked(java.awt.event.MouseEvent evt) {
                try{
                    ...
                    lecturer_name = table.getModel().getValueAt(row, 0).toString();
                    phone = table.getModel().getValueAt(row, 1).toString();
                    gender = table.getModel().getValueAt(row, 2).toString();
    ...
    private void initComponents() {
        ...
        jLabel1.setText("EDIT/DELETE LECTURER INFORMATION");
    ...
    private void deleteActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_deleteActionPerformed
        try{
            int confirm = JOptionPane.showConfirmDialog(this, "Delecte Lecturer Information? Notice that all information related to the lecturer will also be deleted.", "Confirmation", JOptionPane.YES_NO_OPTION);
            PreparedStatement st;
            
            if(confirm == JOptionPane.YES_OPTION){
                String sql = "delete from instructor where instructor_name = ? AND phone = ? AND gender = ?";
                st = DBConnect.con.prepareStatement(sql);

                st.setString(1, lecturer_name);
                st.setString(2, phone);
                st.setString(3, gender);

                st.executeUpdate();
                
                JOptionPane.showMessageDialog(this, "Lecturer information has been deleted","Success",JOptionPane.INFORMATION_MESSAGE);
    ...
    private void editActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_editActionPerformed
        try{
            PreparedStatement st;
            String sql = "select * from instructor where instructor_name = ? AND phone = ? AND gender = ?";
            st = DBConnect.con.prepareStatement(sql);
            ...
            int lecturer_id = DBConnect.rs.getInt("instructor_id");

            this.dispose();
            this.hide();
            
            EditLecturer edit_lecturer = new EditLecturer(lecturer_id);
            desktop1.add(edit_lecturer, new org.netbeans.lib.awtextra.AbsoluteConstraints(0, 0));
            edit_lecturer.setSelected(true);
        }catch (Exception e) {
            System.err.println(e.getMessage());
            e.printStackTrace();
        }
